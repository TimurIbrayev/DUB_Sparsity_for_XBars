...
parser.add_argument('--save_dir',       default='test',         type=str,   help='Name of the subfolder to store logs and checkpoints')
...
parser.add_argument('--lr_scheduler',   default=None,           type=str,   help='Flag to indicate specific learning rate scheduler. Options are: (None, "adaptive", "fixed"). If not specified (None), then the choice is made based on validation split')
...
LOG             = 'GatedVarianceOverHoyer/' + args.save_dir
...
# !!!: Seems redundant (the condition when valid split > 0.0, but MultiStepLR is used... why not have valid_split=0.0? Anyways, the schedule is known! For the case of choosing best msd, just save the last one!)
...
#     if not (VALID_SPLIT  > 0.0):
#         print("Warning! Specified learning rate scheduler type 'adaptive', but validation split is set to 0.0!\n"\
#               "This will result in biased training w.r.t. test set, since learning rate will be adjusted based on the test set loss!")
#     lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=TRAIN['LR_SCHEDULE_GAMMA'], verbose=True)
    
# elif args.lr_scheduler == 'fixed':
#     lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=TRAIN['LR_SCHEDULE'], gamma = TRAIN['LR_SCHEDULE_GAMMA'])
# else:
#     raise ValueError("Given type of learning rate scheduler {} is not found! Options are (None, 'adaptive', and 'fixed')!".format(args.lr_scheduler))
